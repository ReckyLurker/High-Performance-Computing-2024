...
std::vector<double> jacobi(const DenseMatrix& A, const std::vector<double>& B, std::vector<double> x = std::vector<double>(), const double tol = 1e-6, const int max_iter = 1000) {
    ...
    if(x.size() == 0){
        x = std::vector<double>(n_rows, 0.0);
    }
